...
use crate::models::{CalendarDay, DashboardSummary, SavingsTrendPoint};
use crate::queries;
...
#[tauri::command]
pub async fn get_calendar_summary(app: AppHandle, month: String) -> AppResult<Vec<CalendarDay>> {
    let pool = db::pool(&app).await?;
    queries::dashboard::get_calendar_summary(&pool, &month).await
}
